...
use protocol::{ApiKeys, ApiVersion, NodeId, PartitionId, UsableApiVersion, UsableApiVersions, SUPPORTED_API_VERSIONS};
...
impl Broker {
    ...
    pub fn api_version(&self, api_key: ApiKeys) -> Option<ApiVersion> {
        ...
        let supported_version: i16 = SUPPORTED_API_VERSIONS.find(api_key).cloned().unwrap_or(UsableApiVersion {
            min_version: 0,
            max_version: 0,
            api_key: api_key,
        }).max_version;
